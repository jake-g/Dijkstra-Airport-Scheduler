...
public class FindPaths {
	public static void main(String[] args) {
		...
		while(true) {
			...
			// Run shortest path
			Path shortie = g.shortestPath(a, b);
			int cost = shortie.getCost();
			System.out.println("Shortest Path from " + a + " to " + b + ":");

			if (cost == -1) {	// reach dead end...no path
				System.out.println("does not exist");
			} else {
				for (Vertex vertex : shortie.getList()) {
					System.out.print(vertex + " " );
				}
            System.out.println();
            System.out.println(shortie.getCost());	// final cost
			}
		}
	}
